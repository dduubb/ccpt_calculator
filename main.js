// --- tiny reactive store ---------------------------------------------------
  ...
  function setField(id, value, markAsGiven = true){
    const el = document.getElementById(id);
    el.value = value;
    setState(id, value, markAsGiven);
  }
  ...
  function setExemptionBills(bills){
    const tb = $('exRows');
  ...
  // Individual test cases extracted for clarity
  function test1_freezeBaseCalculation(log){
    // 2010 case: Rate 7.188%, Tax $3,097.09, Exemptions (bill): H=431.28, S=287.52, F=3,286.71
    // Expected: AEAV ≈ 43,087; Freeze Base AEAV ≈ 53,087
    setField('rate', '7.188');
    setField('eq', '1.4389');
    setField('tax', '3097.09');

    exemptions = [];
    addExemption('Homeowner', 'H');
    addExemption('Senior', 'S');
    addExemption('Senior Freeze', 'F');
    setExemptionBills([431.28, 287.52, 3286.71]);
    recompute();

    const aeav = Number($('outAEAV').textContent.replace(/[^0-9.]/g, ''));
    const freezeBase = Number($('outFreezeBase').textContent.replace(/[^0-9.]/g, ''));

    log(`Test1 AEAV=${aeav} (expect ~43087)`);
    log(`Test1 FreezeBase=${freezeBase} (expect ~53087)`);
    assert(near(aeav, 43087, 1), 'AEAV not within $1 of 43087', log);
    assert(near(freezeBase, 53087, 1), 'Freeze base not within $1 of 53087', log);
    log('✓ Test1 passed');
  }

  function test2_freezeBaseHidden(log){
    // No Freeze row → Freeze Base should be hidden (—) and no trace step
    setField('rate', '7.000');
    setField('eq', '3.0000');
    setField('tax', '700.00');
    ...
    exemptions = [];
    addExemption('Homeowner', 'H');
    setExemptionBills([350.00]); // → AEAV should be 5000 at 7%
    recompute();

    const freezeBaseDisplay = $('outFreezeBase').textContent.trim();
    const traceText = $('trace').textContent;

    log(`Test2 FreezeBase display='${freezeBaseDisplay}' (expect '—')`);
    assert(freezeBaseDisplay === '—', 'Freeze base should be hidden when no F exemption', log);
    assert(!/"FreezeBaseAEAV"/.test(traceText), 'Trace should not include FreezeBaseAEAV when no F exemption', log);
    log('✓ Test2 passed');
  }

  function test3_noTaxWarningWithMvOnly(log){
    // Enter MV only (no Tax, no Rate) → no tax field warning
    clearField('rate');
    clearField('tax');
    setField('mv', '300000');
    recompute();

    const taxMsg = $('taxMsg').textContent.trim();
    const taxHasBad = $('tax').classList.contains('bad');

    log(`Test3 taxMsg='${taxMsg}', tax.bad=${taxHasBad} (expect '', false)`);
    assert(taxMsg === '' && !taxHasBad, 'Tax field should not warn when only MV is entered', log);
    log('✓ Test3 passed');
  }

  function test4_autoSolveRate(log){
    // Solve Rate from MV + Tax (no exemptions)
    // AEAV=300k*10%*3=90k → rate=7.0%
    clearField('rate');
    setField('eq', '3.0');
    setField('assess', '10');
    exemptions = [];
    renderExemptions();
    setField('mv', '300000');
    setField('tax', '6300.00');
    recompute();

    const rateVal = $('rate').value;
    log(`Test4 solved Rate=${rateVal} (expect 7.000)`);
    assert(rateVal === '7.000', 'Rate should auto-solve to 7.000 from MV & Tax with no exemptions', log);
    log('✓ Test4 passed');
  }

  function test5_autoFillEqFromYear(log){
    // Auto-fill Eq from Year (2010 → 2.9706 per table)
    setField('taxYear', '2010');
    clearField('eq');
    status.set('eq', 'blank');
    maybeAutoFillEq();
    ...
    assert(eqVal === '2.9706', 'Eq should auto-fill to 2.9706 for 2010', log);
    log('✓ Test5 passed');
  }

  function test6_assessmentClassMapping(log){
    // Assessment Class mapping
    const testCases = [
      {classCode: '5', expected: '4'},
      {classCode: '0', expected: '0'},
      {classCode: 'A', expected: '6.6666'},
      {classCode: 'B', expected: '5'}
    ];

    testCases.forEach(({classCode, expected}) => {
      $('classCode').value = classCode;
      $('classCode').dispatchEvent(new Event('change', {bubbles:true}));
      const assessVal = $('assess').value;
      log(`Test6 class ${classCode} → assess=${assessVal} (expect ${expected})`);
      assert(assessVal === expected, `Class ${classCode} should map to ${expected}%`, log);
    });

    log('✓ Test6 passed');
  }

  function test7_derivedFlowsAndLockBehavior(log){
    // Derived flows into inputs; lock behavior
    clearField('av');
    clearField('aav');
    clearField('aeav');
    recompute();

    const avVal = $('av').value;
    const aavVal = $('aav').value;
    const aeavVal = $('aeav').value;

    log(`Test7 derived AV=${avVal}, AAV=${aavVal}, AEAV=${aeavVal}`);
    assert(avVal && aavVal && aeavVal, 'Derived values should populate AV/AAV/AEAV inputs', log);

    // Lock AV by typing a new value and ensure it doesn't get overwritten
    setField('av', '32000.00');
    const avLocked = $('av').value;
    recompute();

    assert($('av').value === avLocked, 'Locked AV should not be overwritten by derived recompute', log);
    log('✓ Test7 passed');
  }
  ...
  function runSelfTests(){
    ...
    const tests = [
      test1_freezeBaseCalculation,
      test2_freezeBaseHidden,
      test3_noTaxWarningWithMvOnly,
      test4_autoSolveRate,
      test5_autoFillEqFromYear,
      test6_assessmentClassMapping,
      test7_derivedFlowsAndLockBehavior
    ];
